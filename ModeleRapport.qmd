...
# Introduction
Notre rapport a pour objectif d'analyser un jeu de données composé de `r dim(gym)[1]` individus et `r dim(gym)[2]` variables. Notre objectif tout au long de ce projet d’examiner la structure des données, d’identifier les relations entre variables et de mettre en évidence des profils caractéristiques.
...
Le jeu de données que nous allons étudier dans ce rapport contient des variables décrivant les profils physiques, les pratiques d’exercice et les mesures de performance des 973 adhérents d'une salle de sport. Les variables sont de différents Qualitatives nominales (**gender**) ou ordinales (**level**);
