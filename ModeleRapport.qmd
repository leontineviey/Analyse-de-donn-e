...
g1=ggplot(gym,aes(x=level,y=water))+
geom_boxplot()+
  labs(
    title = "Distribution du Poids (weight) par Niveau ",
    x = "Niveau",
    y = "eau",
    fill = "Niveau"
  ) +
...
geom_boxplot()+
  labs(
    title = "boxplot de différents variables pour chaque modalité de la variable qualitative **level**",
    x = "Niveau",
    y = "calories",
    fill = "Niveau"
  ) +
...
La @fig-10 représente différents diagrammes en boîte (boxplots) montrant une corrélation positive et marquée entre les variables quantitatives **water**, **calories** et **duration** avec la variable qualitative **level**.
...
De plus, la quantité d'eau consommée est également liée à cette variable. Cela peut s'expliquer par le fait que les sessions sont plus longues, mais également parce que les personnes sportives font plus attention à leur consommation d'eau.
...
ggplot(data = gym) +
  geom_point(aes(x = gender, 
                 ...
                 color = level),
             position = position_jitter(width = 0.3), # Ajoute un léger décalage horizontal pour voir les points
             alpha = 0.7) +
  ...
  labs(color = "Niveau d'Intensité",
       title = "Calories Brûlées et Niveau selon le Genre")
```
Le graphique @fig-11 met en évidence une disparité marquée dans la distribution de la variable "water" selon le genre. On constate un décalage clair : les Hommes présentent des valeurs supérieures et une plus grande dispersion de cet indicateur. Inversement, la répartition pour les Femmes est centrée sur des valeurs inférieures et montre un étalement plus faible.
...
```
La @fig-6 confirme la très forte corrélation entre les variables **duration** et **calories**. Ces observations ne sont pas étonnates car La dépense énergétique (calories) est, par définition physique, une fonction du temps. 
