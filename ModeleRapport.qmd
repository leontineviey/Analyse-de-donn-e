...
Nous allons dans un permier temps comparer 3 méthodes différentes pour mesurer les distances entre 2 groupes différentes. Il existe 3 types de liens entre les classes: lien simple, lien complet ou encore lien moyen.

```{r}


hclustsingle<-hclust(dist(gym), method = "single", members = NULL)
hclustcomplete<-hclust(dist(gym),method = "complete", members = NULL)
#hclustaverage<-hclust(dist(gymq),method = "average", members = NULL)
...
```{r}
hward<-hclust(dist(gym), method = "ward.D2", members = NULL)
...
for (k in 1:10) {
  groupes <- cutree(hclustcomplete, k = k)
