...
1) tracer évolutiom inertie intraclasse

Ce code applique l’algorithme des k-means pour différents nombres de classes (de 2 à 15) et calcule l’inertie intraclasse associée à chaque partition. Le graphique obtenu permet d’utiliser la méthode du coude afin de déterminer un nombre optimal de clusters.
```{r}
...
```
On observe coude soit 3 soit 4
2)critère slihouette
Méthode silhouette: on trouve 3 aussi(METHODE DAISY PEUT GERER VARIABLES MIXTES, J'AI MIS LES QUALITATIVE AUSSI CAR moins de valeur négatives, càd mal placées)  
```{r}
set.seed(3)
...
```
Ce code évalue la qualité des partitions obtenues par k-means à l’aide de l’indice de silhouette pour différents nombres de classes. La silhouette moyenne permet d’identifier le nombre optimal de clusters et complète l’analyse basée sur l’inertie intraclasse. La visualisation de la silhouette fournit une évaluation détaillée de la cohérence interne de chaque cluster.

### K-means: classification non supervisée avec k=3 fixé
-->inertie intraclasse minimale
1)méthode des centres mobiles
2)variante McQueen (mise à jour dun noyau à chzque itération)
3)nuées dynamiques

-->extension kmeans: variables qualitatives et mixtes

1)à K=.. fixé JE N'ARRIVE PAS À METTRE LES CHORDDRIAGRAM
classification avec fonction kmeans
combine de vins par classe?
```{r}
# ACP A completer
resacp=PCA(gym,quali.sup=c(1,8), scale.unit = TRUE,graph=FALSE)
```

-->Mcqueen: le dernier je ne sais pas l'interpréter
```{r}
set.seed(2)

reskmeansMCQUEEN<-kmeans(gymCR[,-c(1,8)],centers=3,iter.max=150,algorithm="MacQueen")

table(reskmeansMCQUEEN$cluster)
fviz_cluster(reskmeansMCQUEEN,data=gymCR[,-c(1,8)],
             ellipse.type="norm",labelsize=8,
             geom=c("point"))+ggtitle("")
fviz_pca_ind(resacp,col.ind=as.factor(reskmeansMCQUEEN$cluster), #importznte de poser as.factor
             geom = c("point"),axes=c(1,2))
# Apelle a un lien avec la qualite
 
table(reskmeansMCQUEEN$cluster, gym$level )
table(reskmeansMCQUEEN$cluster, gym$gender )
...
```
-->méthode centres mobiles QUESTION: faut utiliser les valeurs centrées réduites ou pas?
```{r}
#| eval: false 
set.seed(2)
gymCR<-scale(gym[,-c(1,8)],center=TRUE, scale=TRUE)
...
fviz_cluster(reskmeansFORGY,data=gym[,-c(1,8)],
             ellipse.type="norm",labelsize=8,
             geom=c("point"))+ggtitle("")
fviz_pca_ind(resacp,col.ind=as.factor(reskmeansFORGY$cluster), #importznte de poser as.factor
             geom = c("point"),axes=c(1,2))
# Apelle a un lien avec la qualite
...
set.seed(3)
gymCR<-scale(gym[,-c(1,8)],center=TRUE, scale=TRUE)
reskmeansND<-kmeans(gymCR[,-c(1,8)],centers=3,iter.max=150)

table(reskmeansND$cluster)
fviz_cluster(reskmeansND,data=gymCR[,-c(1,8)],
             ellipse.type="norm",labelsize=8,
             geom=c("point"))+ggtitle("")
fviz_pca_ind(resacp,col.ind=as.factor(reskmeansND$cluster), #importznte de poser as.factor
             geom = c("point"),axes=c(1,2))
# Apelle a un lien avec la qualite
 
table(reskmeansND$cluster, gym$level )
table(reskmeansND$cluster, gym$gender )

clust<-paste("Cl-K",reskmeansND$cluster,sep="")
Tab<-melt(table(clust,gymCR[,1]))
ggplot(Tab,aes(y=value,axis1=clust,axis2=Var2))+
  geom_alluvium(aes(fill=clust))+
  #geom_stratum(width = 1/12)+   
  theme(legend.position = "none")
