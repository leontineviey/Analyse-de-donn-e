...
Le principe de l’analyse en composantes principales (ACP) est de représenter les données dans un espace de dimension plus faible, tout en conservant un maximum d’information issue des données initiales. L’objectif est donc de réduire la dimension du jeu de données en limitant la perte d’information.

Initialement, le nombre de dimensions correspond au nombre de variables quantitatives observées. L’ACP permet de construire de nouvelles variables, appelées *composantes principales*, moins nombreuses que les variables initiales. Chaque composante principale définit un axe du nouvel espace de représentation, appelé *espace factoriel*.
...
On s’intéresse à présent au concept d’**inertie**, notion centrale en analyse en composantes principales, qui permet de quantifier la quantité d’information contenue dans les données et de guider le choix du nombre de dimensions à conserver.
...
Ainsi le premiere axe (Dim1) sera positivement corrélé à la variable *fat* et négativement à *duration*, *calories* et *water*.
Le deuxième axe (Dim2) est négativement corrélé au *bmi* et *weight*.
**À TERMINER DE COMPRENDRE ET CONFIRMER**
